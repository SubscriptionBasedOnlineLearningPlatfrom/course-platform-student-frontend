...
const CourseCard = ({
  ...
  const { getQuizMark, quizMark, setQuizMark } = useContext(CourseContext);
  const [scoreState, setScoreState] = useState(false);

  const viewScore = async (courseId) => {
    await getQuizMark(BackendAPI, courseId);
  };
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-300">
        ...
        <div className="p-6">
          ...
          <button
            ...
            onClick={() => navigate(`/courses/${course.course_id}/content`)}
